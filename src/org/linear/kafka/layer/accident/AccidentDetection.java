...
{
    private static Log log = LogFactory.getLog(AccidentDetection.class);
    private ExecutorService executor;
    private final ConsumerConnector consumer;
    private final String in_comming_topic = "accident_topic";
    private final int numThreads;
    private static boolean shutdownFlag = false;;
    
    public AccidentDetection(){
        Properties props = new Properties();       
        InputStream propertiesIS = AccidentDetection.class.getClassLoader().getResourceAsStream(org.linear.kafka.util.Constants.CONFIG_FILENAME_SEGSTAT);
    ...
    public void run(){
    	...
    	for(final KafkaStream stream : streams){
    		executor.submit(new AccidentDetectionConsumer(stream, threadNumber));
    		threadNumber++;
    	}
    }
    ...
    {    	
    	AccidentDetection segStats = new AccidentDetection();
    	segStats.run();
